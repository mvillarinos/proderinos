...
// Fetch matches
const { data: matches } = await useLazyFetch<Match[]>(`/api/tournaments/${tournamentId}/matches`, {
  server: false,
  default: () => []
})
...
// Page meta
useHead({
  title: computed(() => tournament.value ? `${tournament.value.name} - Proderinos` : 'Tournament - Proderinos')
})
</script>
